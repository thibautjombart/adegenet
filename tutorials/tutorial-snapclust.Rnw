...
\begin{itemize}
 ...
 \item $f_{k,j}$: vector of allele frequencies of group $k$ at locus $j$
 \item $f_{k}$: vector of allele frequencies of group $k$ for all loci, i.e. concatenation $[f_{k,1} \ldots x_{k,J}]$
...
Genetics). The data \texttt{dapcIllus} contains four datasets:
\begin{itemize}
...
% % % % % % % % % % % % % % % % % % % % % % % %
\subsubsection{Looking for hybrids using \texttt{snapclust}}
% % % % % % % % % % % % % % % % % % % % % % % %
