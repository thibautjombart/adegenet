...
The function \texttt{genet.dist} can compute Nei's estimator \cite{tj814} of pairwise $Fst$, defined as:
$$
...
\begin{itemize}
  ...
  \item \texttt{genet.dist} (\textit{hierfstat}): implements pairwise $F_{ST}$, which is also a
  ...
  \item \texttt{dudi.pco} (\textit{ade4}): implements Principal Coordinates Analysis (PCoA); this
    ...
    possible; can be used on outputs of \texttt{dist}, \texttt{dist.genpop}, and \texttt{genet.dist}.
