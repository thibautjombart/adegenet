...
<<include=FALSE>>=
opts_chunk$set(fig.path='figs/base-', fig.keep='high', dev='pdf', fig.width=7, fig.height=7,
...
\begin{abstract}
  ...
  \textit{adegenet} 1.4-0, this tutorial is no longer distributed as a package vignette.
\end{abstract}
...
Make sure you have a recent version of R ($\geq 3.0.3$) by typing:
<<>>=
R.version.string
@
...
\begin{itemize}
  ...
  \item \texttt{loc.names}: a vector of alternative labels for the loci.
  ...
  \item \texttt{all.names}: a vector of alternative labels for the alleles.
  \item \texttt{ind.names}:  a vector of alternative labels for the individuals.
  \item \texttt{pop}: a factor storing group membership of the individuals.
  \item \texttt{pop.names}: a vector of alternative labels for populations.
  \item \texttt{ploidy}: a vector of integer indicating the ploidy of each individual.
  ...
  \item \texttt{strata}: an optional data.frame storing hierarchical structure of individuals.
  \item \texttt{hierarchy}: an optional formula defining the hierarchical structure of individuals.
...
Individual, loci and alleles can have alternative labels (especially when storing long labels which
would make the table @tab unreadable), in which case they are stored in the slots @ind.names,
@loc.names, @all.names.
...
barplot(toto$loc.nall, ylab="Number of alleles",
