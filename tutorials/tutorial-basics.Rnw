...
\begin{itemize}
  ...
  \item \texttt{nPop}: returns the number of loci (SNPs).
...
Note that some of these features might be part of the current devel version of \emph{hierfstat},
...
The basic $F$ statistics are provided by:
<<>>=
library("hierfstat")
fstat(nancycats)
@
...
Are these values significant? This question can be addressed using the G-statistic test \cite{tj511}; it is implemented for \texttt{genind} objects and produces a \texttt{randtest} object (package ade4).
<<>>=
