...
compoplot(res.no.hyb, n.col = 2, col.pal = hybridpal(),
          main = "genclust.em without hybrids")
@
...
compoplot(res2.back, col.pal = hybridpal(),
          main = "genclust.em with F1 and backcrosses")
@
...
The function \texttt{genclust.em.choose.k} permits to run \texttt{genclust.em}
