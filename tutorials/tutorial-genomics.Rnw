...
<<include=FALSE>>=
opts_chunk$set(fig.path='figs/genomics-', fig.keep='high', dev='pdf', fig.width=7, fig.height=7,
...
The new version of \texttt{glSim} contains new optional arguments to allow for greater complexity in the simulated data.
\\
Contrasting structures between two groups of individuals can be generated by specifying the parameters \texttt{n.snp.struc},
...
\\
In addition, the user has the option to simulate background group structure between \texttt{k} ancestral populations of relative size \texttt{pop.freq}, generated by altering allele frequencies in the 'non-structural' SNPs. The factorisation of individuals into these \texttt{k} groups will then
...
\\
The non-structural SNPs, with or without ancestral population structure, can also be generated with or without linkage disequilibrium (LD).
...
\\
See \texttt{?glSim} for more details.
