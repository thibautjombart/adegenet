...
#' 
#' The Monmonier's algorithm detects boundaries among vertices of a valuated
#' graph. This is achieved by finding the path exhibiting the largest distances
#' between connected vertices.\cr
#' 
#' The highest distance between two connected vertices (i.e. neighbours) is
#' found, giving the starting point of the path. Then, the algorithm seeks the
#' highest distance between immediate neighbours, and so on until a threshold
#' value is attained.  This threshold can be chosen from the plot of sorted
#' distances between connected vertices: a boundary will likely result in an
#' abrupt decrease of these values.\cr
#' 
#' When several paths are looked for, the previous paths are taken into
#' account, and cannot be either crossed or redrawn. Monmonier's algorithm can
#' be used to assess the boundaries between patches of homogeneous
#' observations.\cr
#' 
#' Although Monmonier algorithm was initially designed for Voronoi tesselation,
#' this implementation generalizes this algorithm to different connection
#' networks. The \code{optimize.monmonier} function produces a \code{monmonier}
#' object by trying several starting points, and returning the best boundary
#' (i.e. largest sum of local distances). This is designed to avoid the
#' algorithm to be trapped by a single strong local difference inside an
#' homogeneous patch.
#' 
#' The function \code{monmonier} returns a list of the class \code{monmonier},
#' which contains the general informations about the algorithm, and about each
#' run. When displayed, the width of the boundaries reflects their 'strength'.
...
#' 
#' As there is no perfect method to display graphically a quantitative variable
#' (see for instance the differences between the two methods of
#' \code{s.value}), the boundaries provided by this algorithm seem sometimes
#' more reliable than the boundaries our eyes perceive (or miss).\cr
#' 
#' @aliases monmonier optimize.monmonier plot.monmonier print.monmonier
#' @param xy a matrix yielding the spatial coordinates of the objects, with two
#' columns respectively giving X and Y
#' @param dist an object of class \code{dist}, giving the distances between the
#' objects
#' @param cn a connection network of class \code{nb} (package \code{spdep})
#' @param threshold a number giving the minimal distance between two neighbours
#' crossed by the path; by default, this is the third quartile of all the
#' distances between neighbours
#' @param bd.length an optional integer giving the requested length of the
#' boundaries (in number of local differences)
#' @param nrun is a integer giving the number of runs of the algorithm, that
#' is, the number of paths to search, being one by default
#' @param skip.local.diff is a vector of integers, whose length is the number
#' of paths (\code{nrun}); each integer gives the number of starting point to
#' skip, to avoid being stuck in a local difference between two neighbours into
#' an homogeneous patch; none are skipped by default
#' @param scanthres a logical stating whether the threshold sould be chosen
#' from the barplot of sorted distances between neighbours
#' @param allowLoop a logical specifying whether the boundary can loop (TRUE,
#' default) or not (FALSE)
#' @param ntry an integer giving the number of different starting points tried.
#' @param return.best a logical stating whether the best monmonier object
#' should be returned (TRUE, default) or not (FALSE)
#' @param display.graph a logical whether the scores of each try should be
#' plotted (TRUE, default) or not
#' @param x a monmonier object
#' @param variable a variable to be plotted using \code{s.value} (package
#' \code{ade4})
#' @param displayed.runs an integer vector giving the rank of the paths to
#' represent
#' @param add.arrows a logical, stating whether arrows should indicate the
#' direction of the path (TRUE) or not (FALSE, used by default)
#' @param col a characters vector giving the colors to be used for each
#' boundary; recycled is needed; 'blue' is used by default
#' @param lty a characters vector giving the type of line to be used for each
#' boundary; 1 is used by default
#' @param bwd a number giving the boundary width factor, applying to every
#' segments of the paths; 4 is used by default
#' @param clegend like in \code{s.value}, the size factor of the legend if a
#' variable is represented
#' @param csize like in \code{s.value}, the size factor of the squares used to
#' represent a variable
#' @param method like in \code{s.value}, a character giving the method to be
#' used to represent the variable, either 'squaresize' (by default) or
#' 'greylevel'
#' @param sub a string of characters giving the subtitle of the plot
#' @param csub the size factor of the subtitle
#' @param possub the position of the subtitle; available choices are 'topleft'
#' (by default), 'topright', 'bottomleft', and 'bottomright'
#' @param cneig the size factor of the connection network
#' @param pixmap an object of the class \code{pixmap} displayed in the map
#' background
#' @param contour a data frame with 4 columns to plot the contour of the map:
#' each row gives a segment (x1,y1,x2,y2)
#' @param area a data frame of class 'area' to plot a set of surface units in
#' contour
#' @param add.plot a logical stating whether the plot should be added to the
#' current one (TRUE), or displayed in a new window (FALSE, by default)
#' @param \dots further arguments passed to other methods
#' @return Returns an object of class \code{monmonier}, which contains the
#' following elements : \item{run1 (run2, }{for each run, a list containing a
#' dataframe giving the path coordinates, and a vector of the distances between
#' neighbours of the path}\item{list()}{for each run, a list containing a
#' dataframe giving the path coordinates, and a vector of the distances between
#' neighbours of the path}\item{)}{for each run, a list containing a dataframe
#' giving the path coordinates, and a vector of the distances between
#' neighbours of the path} \item{nrun}{the number of runs performed, i.e. the
#' number of boundaries in the monmonier object} \item{threshold}{the threshold
#' value, minimal distance between neighbours accounted for by the algorithm}
#' \item{xy}{the matrix of spatial coordinates} \item{cn}{the connection
#' network of class \code{nb}} \item{call}{the call of the function}
#' @author Thibaut Jombart \email{t.jombart@@imperial.ac.uk}
#' @seealso \code{\link{spca}},\code{\link[spdep]{edit.nb}}
#' @references Monmonier, M. (1973) Maximum-difference barriers: an alternative
#' numerical regionalization method. \emph{Geographic Analysis}, \bold{3},
#' 245--261.
#' 
...
#' "Monmonier's algorithm". \emph{Human Biology}, \bold{76}, 173--190
