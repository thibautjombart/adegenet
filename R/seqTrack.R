...
#' haplotypes or genotypes for which a collection date is available. Contrary
#' to phylogenetic methods which aims at reconstructing hypothetical ancestors
#' for observed sequences, SeqTrack considers that ancestors and descendents
#' are sampled together, and therefore infers ancestry relationships among the
#' sampled sequences.
#' 
#' This approach proved more efficient than phylogenetic approaches for
#' reconstructing transmission trees in densely sampled disease outbreaks [1].
#' This implementation defines a generic function \code{seqTrack} with methods
#' for specific object classes.
#' 
#' === Maximum parsimony genealogies ===\cr Maximum parsimony genealogies can
#' be obtained easily using this implementation of seqTrack. One has to provide
#' in \code{x} a matrix of genetic distances. The most straightforward distance
#' is the number of differing nucleotides. See \code{\link[ape]{dist.dna}} in
#' the ape package for a wide range of genetic distances between aligned
#' sequences. The argument \code{best} should be set to "min" (its default
#' value), so that the identified genealogy minimizes the total number of
#' mutations. If \code{x} contains number of mutations, then \code{mu} and
#' \code{haplo.length} should also be provided for resolving ties in equally
#' parsimonious ancestors using maximum likelihood.
#' 
#' === Likelihood of observed genetic differentiation ===\cr The probability of
#' oberving a given number of mutations between a sequence and its ancestor can
#' be computed using \code{get.likelihood.seqTrack}. Note that this is only
#' possible if \code{x} contained number of mutations.
#' 
#' === Plotting/converting seqTrack objects to graphs ===\cr seqTrack objects
#' are best plotted as graphs. From adegenet_1.3-5 onwards, seqTrack objects
#' can be converted to \code{igraph} objects (from the package \code{igraph}),
#' which can in turn be plotted and manipulated using classical graph tools.
#' The plot method does this operation automatically, using colors to represent
#' edge weights, and using time-ordering of the data from top (ancient) to
#' bottom (recent).
#' 
#' @aliases seqTrack seqTrack.default seqTrack.matrix plotSeqTrack
#' get.likelihood get.likelihood.seqTrack seqTrack-class as.igraph.seqTrack
#' plot.seqTrack
#' @param x for seqTrack, a matrix giving weights to pairs of ancestries such
#' that x[i,j] is the weight of 'i ancestor of j'. For plotSeqTrack and
#' get.likelihood. seqTrack, a \code{seqTrack} object.
#' @param x.names a character vector giving the labels of the
#' haplotypes/genotypes
#' @param x.dates a vector of collection dates for the sampled
#' haplotypes/genotypes. Dates must have the POSIXct format. See \code{details}
#' or \code{?as.POSIXct} for more information.
#' @param best a character string matching 'min' or 'max', indicating whether
#' genealogies should minimize or maximize the sum of weights of ancestries.
#' @param prox.mat an optional matrix of proximities between
#' haplotypes/genotypes used to resolve ties in the choice of ancestors, by
#' picking up the 'closest' ancestor amongst possible ancestors, in the sense
#' of \code{prox.mat}. \code{prox.mat[i,j]} must indicate a proximity for the
#' relationship 'i ancestor to j'. For instance, if \code{prox.mat} contains
#' spatial proximities, then \code{prox.mat[i,j]} gives a measure of how easy
#' it is to migrate from location 'i' to 'j'.
#' @param mu (optional) a mutation rate, per site and per day. When 'x'
...
#' @param haplo.length (optional) the length of analysed sequences in number of
#' nucleotides. When 'x' contains numbers of mutations, used to resolve ties
#' using a maximum likelihood approach (requires \code{mu} to be provided).
...
#' inversed by default.
#' @param xy spatial coordinates of the sampled haplotypes/genotypes.
#' @param use.arrows a logical indicating whether arrows should be used to
#' represented ancestries (pointing from ancestor to descendent, TRUE), or
#' whether segments shall be used (FALSE).
#' @param annot a logical indicating whether arrows or segments representing
#' ancestries should be annotated (TRUE) or not (FALSE).
#' @param labels a character vector containing annotations of the ancestries.
#' If left empty, ancestries are annotated by the descendent.
#' @param col a vector of colors to be used for plotting ancestries.
#' @param bg a color to be used as background.
#' @param add a logical stating whether the plot should be added to current
#' figure (TRUE), or drawn as a new plot (FALSE, default).
#' @param quiet a logical stating whether messages other than errors should be
#' displayed (FALSE, default), or hidden (TRUE).
#' @param date.range a vector of length two with POSIXct format indicating the
#' time window for which ancestries should be displayed.
#' @param jitter.arrows a positive number indicating the amount of noise to be
#' added to coordinates of arrows; useful when several arrows overlap. See
#' \code{\link{jitter}}.
#' @param plot a logical stating whether a plot should be drawn (TRUE,
#' default), or not (FALSE). In all cases, the function invisibly returns
#' plotting information.
#' @param list() further arguments to be passed to other methods
#' @return === output of seqTrack ===\cr seqTrack function returns data.frame
#' with the class \code{seqTrack}, in which each row is an inferred ancestry
#' described by the following columns: - id: indices identifying
#' haplotypes/genotypes\cr - ances: index of the inferred ancestor\cr - weight:
#' weight of the inferred ancestries\cr - date: date of the
#' haplotype/genotype\cr - ances.date: date of the ancestor
#' 
#' === output of plotSeqTrack ===\cr This graphical function invisibly returns
#' the coordinates of the arrows/segments drawn and their colors, as a
#' data.frame.
#' @author Thibaut Jombart \email{t.jombart@@imperial.ac.uk}
#' @seealso \code{\link[ape]{dist.dna}} in the ape package to compute pairwise
