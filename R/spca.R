...
#' 
#' These functions are designed to perform a spatial principal component
#' analysis and to display the results. They call upon \code{multispati} from
#' the \code{ade4} package.
#' 
#' \code{spca} performs the spatial component analysis. Other functions are:\cr
#' 
#' - \code{print.spca}: prints the spca content\cr
#' 
#' - \code{summary.spca}: gives variance and autocorrelation\cr statistics
#' 
#' - \code{plot.spca}: usefull graphics (connection network, 3 different
#' representations of map of scores, eigenvalues barplot and decomposition)\cr
#' 
#' - \code{screeplot.spca}: decomposes spca eigenvalues into variance and
#' autocorrelation\cr
#' 
#' - \code{colorplot.spca}: represents principal components of sPCA in space
#' using the RGB system.\cr
#' 
#' A tutorial describes how to perform a sPCA: see
#' \url{http://adegenet.r-forge.r-project.org/files/tutorial-spca.pdf} or type
#' \code{adegenetTutorial(which="spca")}.
#' 
#' The spatial principal component analysis (sPCA) is designed to investigate
#' spatial patterns in the genetic variability. Given multilocus genotypes
#' (individual level) or allelic frequency (population level) and spatial
#' coordinates, it finds individuals (or population) scores maximizing the
#' product of variance and spatial autocorrelation (Moran's I). Large positive
#' and negative eigenvalues correspond to global and local structures.\cr
#' 
#' Spatial weights can be obtained in several ways, depending how the arguments
#' \code{xy}, \code{cn}, and \code{matWeight} are set.\cr When several
#' acceptable ways are used at the same time, priority is as follows:\cr
#' \code{matWeight} > \code{cn} > \code{xy} \cr
#' 
#' @aliases spca print.spca summary.spca plot.spca screeplot.spca
#' colorplot.spca
#' @param obj a \code{genind} or \code{genpop} object.
#' @param xy a matrix or data.frame with two columns for x and y coordinates.
#' Seeked from obj\$other\$xy if it exists when xy is not provided. Can be NULL
#' if a \code{nb} object is provided in \code{cn}.\cr Longitude/latitude
#' coordinates should be converted first by a given projection (see 'See Also'
#' section).
#' @param cn a connection network of the class 'nb' (package spdep). Can be
#' NULL if xy is provided. Can be easily obtained using the function chooseCN
#' (see details).
#' @param matWeight a square matrix of spatial weights, indicating the spatial
#' proximities between entities. If provided, this argument prevails over
#' \code{cn} (see details).
#' @param scale a logical indicating whether alleles should be scaled to unit
...
#' ('local structures').
#' @param type an integer giving the type of graph (see details in
...
#' @param truenames a logical stating whether true names should be used for
...
#' @return The class \code{spca} are given to lists with the following
...
#' data.frame of alleles loadings for each axis.} \item{li}{a data.frame of row
#' (individuals or populations) coordinates onto the sPCA axes.} \item{ls}{a
#' data.frame of lag vectors of the row coordinates; useful to clarify maps of
#' global scores .} \item{as}{a data.frame giving the coordinates of the PCA
#' axes onto the sPCA axes.} \item{call}{the matched call.} \item{xy}{a matrix
#' of spatial coordinates.} \item{lw}{a list of spatial weights of class
#' \code{listw}.}
#' 
#' Other functions have different outputs:\cr - \code{summary.spca} returns a
#' list with 3 components: \code{Istat} giving the null, minimum and maximum
#' Moran's I values; \code{pca} gives variance and I statistics for the
#' principal component analysis; \code{spca} gives variance and I statistics
#' for the sPCA.\cr
#' 
...
#' @seealso \code{\link{spcaIllus}} and \code{\link{rupica}} for datasets
#' illustrating the sPCA \cr \code{\link{global.rtest}} and
#' \code{\link{local.rtest}} \cr \code{\link{chooseCN}},
#' \code{\link[ade4]{multispati}}, \code{\link[ade4]{multispati.randtest}}\cr
#' \code{convUL}, from the package 'PBSmapping' to convert longitude/latitude
#' to UTM coordinates.
...
#' 263--283.
#' 
#' Moran, P.A.P. (1948) The interpretation of statistical maps. \emph{Journal
#' of the Royal Statistical Society, B} \bold{10}, 243--251.
#' 
