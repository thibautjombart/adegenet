...
#' 
#' The function \code{snpzip} identifies the set of alleles which contribute
#' most significantly to phenotypic structure.
#' 
#' This procedure uses Discriminant Analysis of Principal Components (DAPC) to
#' quantify the contribution of individual alleles to between-population
#' structure. Then, defining contribution to DAPC as the measure of distance
#' between alleles, hierarchical clustering is used to identify two groups of
#' alleles: structural SNPs and non-structural SNPs.
#' 
#' \code{snpzip} provides an objective procedure to delineate between
#' structural and non-structural SNPs identified by Discriminant Analysis of
#' Principal Components (DAPC, Jombart et al. 2010).  \code{snpzip} precedes
#' the multivariate analysis with a cross-validation step to ensure that the
#' subsequent DAPC is performed optimally.  The contributions of alleles to the
#' DAPC are then submitted to \code{hclust}, where they define a distance
#' matrix upon which hierarchical clustering is carried out.  To complete the
#' procedure, \code{snpzip} uses \code{cutree} to automatically subdivide the
#' set of SNPs fed into the analysis into two groups: those which contribute
#' significantly to the phenotypic structure of interest, and those which do
#' not.
#' 
#' @param snps a snps \code{matrix} used as input of DAPC.
#' @param y either a \code{factor} indicating the group membership of
#' individuals, or a dapc object.
#' @param plot a \code{logical} indicating whether a graphical representation
#' of the DAPC results should be displayed.
#' @param xval.plot a \code{logical} indicating whether the results of the
#' cross-validation step should be displayed (iff \code{y} is a factor).
#' @param loading.plot a \code{logical} indicating whether a loading.plot
#' displaying the SNP selection threshold should be displayed.
#' @param method the clustering method to be used. This should be (an
#' unambiguous abbreviation of) one of \code{"complete", "single", "average",
#' "centroid", "mcquitty", "median",} or \code{"ward"}.
#' @param \dots further arguments.
#' @return A \code{list} with four items if \code{y} is a factor, or two items
#' if \code{y} is a dapc object: The first cites the number of principal
#' components (PCs) of PCA retained in the DAPC.
#' 
#' The second item is an embedded list which first indicates the number of
#' structural and non-structural SNPs identified by \code{snpzip}, second
#' provides a list of the structuring alleles, third gives the names of the
#' selected alleles, and fourth details the contributions of these structuring
#' alleles to the DAPC.
#' 
#' The optional third item provides measures of discrimination success both
#' overall and by group.
#' 
#' The optional fourth item contains the dapc object generated if \code{y} was
#' a factor.
#' 
#' If \code{plot=TRUE}, a scatter plot will provide a visualization of the DAPC
#' results.
#' 
#' If \code{xval.plot=TRUE}, the results of the cross-validation step will be
#' displayed as an \code{array} of the format generated by xvalDapc, and a
#' scatter plot of the results of cross-validation will be provided.
#' 
#' If \code{loading.plot=TRUE}, a loading plot will be generated to show the
#' contributions of alleles to the DAPC, and the SNP selection threshold will
#' be indicated.  If the number of Discriminant Axes (\code{n.da}) in the DAPC
#' is greater than 1, \code{loading.plot=TRUE} will generate one loading plot
#' for each discriminant axis.
