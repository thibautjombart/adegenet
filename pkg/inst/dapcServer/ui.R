...
tabPanel("Help", 
         ...
         h3("Overview"),
         helpText("Welcome to the DAPC Server Help Section. Under each section heading below you can find a brief description and useful information
         ...
         helpText("The DAPC Server aims to provide a user-friendly interactive application of some of the functions contained in the R package adegenet.
                  More information about adegenet can be found in the following resources:"),
         
         ###### really would like to get this to hyperlink:
         helpText("http://adegenet.r-forge.r-project.org/"),
         helpText("Jombart, Thibaut. adegenet: a R package for the multivariate analysis of genetic markers. 
                  Bioinformatics (Oxford, England), 24(11):1403. 5, June 2008."),
         
         helpText("On the DAPC Server, users can explore the Discriminant Analysis of Principal Components (DAPC) method.
         ...
         helpText("DAPC is a supervised method, which means that the clusters of individuals to be analysed must be pre-defined by the user.
         ...
         helpText("For any dataset containing a set of genetic variables for any number of individuals and a population grouping factor,
         ...
         helpText("Acceptable file types for input datasets include 'gtx/gen/dat/GTX/GEN/DAT/RData/Rdata/Rda/rda' and 'GENETIX/genepop/Fstat/R data'."),
                  

         ## SCATTER PLOT ##
         
         h3("Scatter Plot"),
         helpText("The Scatter Plot page provides a visual assessment of between-population differentiation.
         ...
         helpText("The number of axes retained in both the PCA and DA steps of DAPC will have an impact on the 
         ...
         helpText("There are a wide variety of graphical parameters for the DAPC scatterplot that can be customised
         ...
         h3("Summary"),
         helpText("This page provides a summary of the dapc object."),
         helpText("$n.dim' indicates the number of retained DAPC axes, which is affected by both the number of PCA
                  axes and DA axes retained."),
         helpText("'$n.pop' indicates the number of groups or populations, which is defined by the dataset."),
         helpText("'$assign.prop' indicates the proportion of overall correct assignment"),
         helpText("'$assign.per.pop' indicates the proportions of successful reassignment (based on the discriminant
         ...
         helpText("'$prior.grp.size' indicates prior group sizes."),
         helpText("'$post.grp.size' indicates posterior group sizes."),
         ...
         h3("Compoplot"),
         helpText("This page displays a compoplot, which is a bar plot showing the probabilities of assignment of 
         ...
         h3("Loading Plot"),
         helpText("The Loading Plot page allows the user to examine how the original variables contribute to the 
         ...
         helpText("The side panel on the Loading Plot page provides the option of selecting a threshold above which variables are identified. 
         ...
         h3("Cross-validation"),
         helpText("When the 'Perform cross-validation?' box is ticked, this optimisation procedure will be carried
                  out on the Cross-validation page."),
         helpText("Cross-validation is an optimisation procedure that is used in the context of DAPC to identify the number of principal components
         ...
         helpText("A scatterplot of the results is displayed, showing the number of PCs retained on the x-axis and
                  success on the y-axis. Individual replicates appear as dots, and the density of points is displayed
                  in blue."),
         helpText("Ideally, the data should fall in an arc, indicating an optimal point at its maximum where
                  the number of PCs retained leads to better predictive success than numbers of PCs either
                  above or below."),
         helpText("Below the plot, a variety of summary statistics are provided. 
...
tabPanel("Glossary",
         h3("Compoplot"),
         helpText("A compoplot is a bar plot showing the probabilities of assignment of individuals to the different clusters."),
         h3("Cross-validation"),
         helpText("Cross-validation is an optimisation procedure that is used in the context of DAPC to identify the number of principal components
                  that gives rise to the model with the highest predictive capacity. In cross-validation for DAPC, the data is divided into a training
                  set and a validation set (by default, comprising 90% and 10% of the data, respectively). The analysis is run on the training set with
         ...
         helpText("Note: Performing cross-validation can substantially improve the results of DAPC; however, the amount of computational time 
                  required increases with the size of the dataset in question and the number of replicates carried out."),
         h3("DA"),
         helpText("Discriminant Analysis (DA) is a procedure for optimally describing the differences between groups of individuals.
         ...
         h3("DA axis"),
         helpText("DAPC uses Discriminant Analysis (DA) to describe the differences between K groups of individuals along 
                  a maximum of (K - 1) Discriminant Analysis axes (DA axes)."),
         h3("DAPC"),
         helpText("Discriminant Analysis of Principal Components (DAPC) is a multivariate method that uses genetic data to describe the
         ...
         helpText("For more, see: Jombart T, Devillard S and Balloux F (2010) Discriminant analysis of principal components: a new method for the 
                  analysis of genetically structured populations. BMC Genetics11:94. doi:10.1186/1471-2156-11-94."),
         h3("Inertia ellipse"),
         helpText("The inertia ellipses displayed optionally on the DAPC scatter plot provide graphical summaries of a cloud of points. 
         ...
         h3("Loading"),
         helpText("Loadings provide a measure of the contribution of each original variable to the discrimination between groups along a given
         ...
         helpText("A minimum spanning tree is a graph theoretical phenomenon in which all of the vertices (in our case, cluster centroids) of
                  a graph are connected into the tree that contains the shortest set of possible paths between vertices."),
         h3("PCA"),
         helpText("Principal Component Analysis (PCA) is a multivariate statistical method that generates linearly 
         ...
         h3("PC, PCA axis"),
         helpText("Principal components (PCs, or PCA axes) are sets of linearly uncorrelated synthetic variables composed of 
         ...
         h3("RMSE"),
         helpText("The root mean squared error (RMSE) is a measure of the error of an estimator. On the DAPC Server, RMSE is used in 
         ...
         helpText("RMSE = sqrt((1/n)*sum(i=1 to n)(Yhat.i - Yi)^2), where Yhat is a vector of n predictions,
                  and Y is the vector of the true values."),
         h3("Screeplot"),
         helpText("A graphical representation of the variance, or cumulative variance, contained in the set of principal components 
                  or discriminant functions. Components shaded in black represent those that have been retained in the analysis."),
         h3("Training set"),
         helpText("The training set, in cross-validation, is the set of individuals retained in the analysis. 
